...
define(['angular','directiveModule'],function(angular, app){
    app.directive('fileUpload', ["$timeout", "uploadService", function($timeout, uploadService) {
        ...
        return {
            ...
            template: 
                '<a class="pic-item single" ng-repeat="src in uploadImgs track by $index" href="javascript:void(0);">' +
                    '<input type="file" ng-if="limit==1" accept="image/*" ng-if="!isMultiple" />' +
                    '<span><img class="bt-pic" ng-model="src" ng-click="previewPic($index)"/></span><i class="iconfont icon-shanchu ks-cPrimary" ng-if="limit>1" ng-click="del($index)"></i>' +
                '</a>' +
            ...
            link: function ($scope, element, attrs, ngModel) {
                ...
                ngModel.$render = function(){
                    ...
                    $scope.isMultiple = false;//limit > 1;
                                            ...
                                            if(base64Str){
                                                $timeout(function(){
                                                    if($scope.uploadImgs.length < limit){
                                                        uploadService.upload(base64Str).then(function(data){
                                                            if(data && data.url){
                                                                $scope.uploadImgs.push(data.url);
                                                                getIsEnd();
                                                            } else{
                                                                getIsEnd();
                                                            }
