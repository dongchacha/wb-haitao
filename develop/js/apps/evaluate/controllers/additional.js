 ...
 * 追加评价
 */
define(["services/orderService", "services/evaluateService"
], function(){
    ...
    function ($scope, $rootScope, $state, $stateParams, $timeout, orderService, evaluateService) {  
        ...
        $scope.submit = function(){
            ...
            if(isValid){
                evaluateService.addEvaluateAgain($scope.order.order_id, $scope.evalute_data).then(function(){
                    new Say({
                        title: "追加成功",
                        onhide: function(){
                            $scope.goBack();
