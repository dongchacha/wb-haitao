...
define(["jquery"], function($){
	...
	// 默认配置。
		DEFAULT_CONFIG = {
			...
			imgFadeOut: true, 		//图片淡入
			isScale: false,			//是否缩放切换
		...
		Fn = function (oConfig) {
			...
			_self.nContainerWidth = $(window).width();	//容器默认宽度
	...
	$.extend(Fn.prototype, {
		...
		_resizeContainer: function(){
			...
			function _resize(){
				if(_self.config.fixedWidth){
					...
					_self.nContainerWidth = _self.wrapper.innerWidth() || $(window).width();
				}
			...
			$(window).resize(function(){
				...
				resizeTimer = setTimeout(function(){
					...
					_self._renderContainer.call(_self);
					_self._fixPicToCenter.call(_self);
				}, 100);
			});
			_self._renderContainer.call(_self);
			return _self;
		...
		_renderContainer: function(){
			var _self = this,
				fWidthRate = _self.config.isFlexable ? $(window).width() / 375 : 1,
		...
		_initRender: function(){
			...
			if(_self.config.isLoop){
				...
				// 	$timeout(function(){
				// 	    var scope = angular.element(jLastClone).scope();
				// 	    $compile(jLastClone)(scope);
				// 	    $compile(jFirstClone)(scope);
				// 	}, 0);
				// }]);
				//        	}
			}
		 ...
		 * 图片居中显示
		 */
		_fixPicToCenter: function(){
			var _self = this,
				jImgs = _self.slideUl.find("li img")
			;

			_self._renderPicItem();
			jImgs.each(function(index){
				...
				var img = new Image();
				if(jThis.data("orignWidth")){
					img.width = jThis.data("orignWidth");
					img.height = jThis.data("orignHeight");
					fnCallback();
					return;
				}
				...
				if(img.complete) {// 如果图片已经存在于浏览器缓存，直接调用回调函数
					jThis
						.data("orignWidth", img.width)
						.data("orignHeight", img.height)
					;
					fnCallback();
				...
				function fnCallback(){
					if(img.width > nMaxWidth || img.height > nMaxHeight){
						if(_self.config.picFixed){//图片适配
							if(img.width / img.height >= nMaxWidth / nMaxHeight){
		...
		_addEventListener: function() {
			...
			function _fnTouchMove(e) {
				...
				//手指离开之后，手开始到结束的距离
				_moveX = oMove.x - _startX;
				_moveY = oMove.y - _startY;
				...
				var nLeft = _self.moveLeft + _moveX;
			...
			function _fnTouchEnd(e) {
				...
				nDur = new Date().getTime() - _startTime //执行的时间
				;

				if(nDur > 200 || (Math.abs(_moveX) > nDistance || Math.abs(_moveY) > nDistance)){
					isTap = false;
				}
				...
				function goToEnd(){
					if(isTap && !_self.isAnimating){//点击触碰事件
						var nShowIndex = _self._getTrueCurrIndex();

						//预览
						if(_self.config.canPreview){
							if(window.wv && wv.getBridgeListenState() != 2){//app端
								var asUrls = [];
								_self.originalSildeItems.find("img").each(function(){
									asUrls.push($(this).attr("src"));
								});
								wv.showPhoto(asUrls, nShowIndex, true);
							} else{
								require(["photoSwipe"], function(PhotoSwipe){
									...
									new PhotoSwipe({
										...
										closeCallback: function(index){
											// _self.showIndex = index + 1;
											_self.showPic(false, index + 1);
											_self.autoPlay();
										}
									});
								});
