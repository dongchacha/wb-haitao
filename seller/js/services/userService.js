...
define(["serviceModule", "md5"], function(app) {
    app.factory("userService", ["$q", "$http", "$state", function($q, $http, $state) {
        ...
        var services = {
            ...
            logout: function(){
                var deferred = $q.defer(),
                    URL = "../m/index.html#/login",   //正式接口地址/webapi/api/user/logout
                ...
                $http({
                    ...
                    location.href = "../m/index.html#login?prevUrl=" + Util.encodeUri(location.href);
            ...
            getValidCode: function(mobile){
                var deferred = $q.defer(),
                    URL = "/webapi/index.php?cls=login&fun=getloginverifycode",   //正式接口地址
                    TEST_URL = "json/user/getUserInfo.txt"   //测试接口地址
                ;
             ...
             * @return {Object} promise
             */
            getModifyMobileValidCode: function(mobile){
                var deferred = $q.defer(),
                    URL = "/webapi/index.php?cls=seller_password&fun=getCode",   //正式接口地址
                    TEST_URL = "json/user/getUserInfo.txt"   //测试接口地址
                ;
             ...
             * 更改手机
             */
            modifyMobile: function(model){
                var deferred = $q.defer(),
                    URL = "/webapi/store/index.php?cls=password&fun=change_mobile",   //正式接口地址
                    TEST_URL = "json/user/getUserInfo.txt"   //测试接口地址
                ;
                ...
                $http({
                    ...
                    data: {
                        mobile: model.mobile,
                        code: $.md5(model.verifycode + $.md5(model.password))
                    }
                }).success(function(data){
                    if(data.code != 200){
                        new Say(data.message);
